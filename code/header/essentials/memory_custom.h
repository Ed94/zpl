...
ZPL_DEF void pool_init_align( zpl_pool* pool, zpl_allocator backing, sw num_blocks, sw block_size, sw block_align );
...
ZPL_IMPL_INLINE void* default_resize_align( zpl_allocator a, void* old_memory, sw old_size, sw new_size, sw alignment )
{
	if ( ! old_memory )
...
ZPL_IMPL_INLINE void arena_check( zpl_arena* arena )
{
