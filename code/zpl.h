...
Usage:
#  define ZPL_IMPLEMENTATION exactly in ONE source a_file right BEFORE including the library, like:
...
#  include "zpl.h"
  ...
  Some of these modules used to depend on printf, but they use the standard library if the distro is enabled now.
...
#			else
#				define zpl__strlen                   strlen
#				define zpl__printf_err( fmt, ... )   printf_err( fmt, __VA_ARGS__ )
#				define zpl__printf_err_va( fmt, va ) printf_err_va( fmt, va )
#			endif
#		endif
